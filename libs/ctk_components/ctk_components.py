...
ICON_PATH = {
    ...
    "eye1": (
        os.path.join(ICON_DIR, "eye1_black.png"),
        os.path.join(ICON_DIR, "eye1_white.png"),
    ),
    "eye2": (
        os.path.join(ICON_DIR, "eye2_black.png"),
        os.path.join(ICON_DIR, "eye2_white.png"),
    ),
...
btn_footer = {
    **btn,
    "fg_color": ("#EBECF0", "#393B40"),
    "hover_color": ("#DFE1E5", "#43454A"),
    "corner_radius": 0,
}

DEFAULT_ICON_ONLY_BTN = {
    **DEFAULT_BTN,
    "height": 50,
    "text_color": ("#000000", "#FFFFFF"),
    "anchor": "center",
...
btn_icon_only_active = {
    **btn_icon_only,
    "fg_color": (ctk.ThemeManager.theme["CTkButton"]["fg_color"]),
    "hover": True,
}
btn_icon_only_footer = {
    **DEFAULT_ICON_ONLY_BTN,
    "width": 80,
    "fg_color": ("#EBECF0", "#393B40"),
    "hover_color": ("#DFE1E5", "#43454A"),
    ...
    def __init__(
        ...
        if state not in ICON_PATH or ICON_PATH[state] is None:
            self.icon = ctk.CTkImage(
                Image.open(ICON_PATH["info"]), Image.open(ICON_PATH["info"]), (30, 30)
            )
        else:
            self.icon = ctk.CTkImage(
                Image.open(ICON_PATH[state]), Image.open(ICON_PATH[state]), (30, 30)
            )
        ...
        self.message.grid(
            row=1, column=0, padx=(20, 10), pady=10, sticky="nsew", columnspan=2
        )
...
class CTkBanner(ctk.CTkFrame):
    def __init__(
        ...
        if state not in ICON_PATH or ICON_PATH[state] is None:
            ...
            self.icon = ctk.CTkImage(
                Image.open(ICON_PATH[state]), Image.open(ICON_PATH[state]), (24, 24)
            )
        ...
        self.title_label = ctk.CTkLabel(
            self, text=f"  {title}", font=("", 16), image=self.icon, compound="left"
        )
        ...
        self.btn_1 = ctk.CTkButton(
            self, text=btn1, **LINK_BTN, command=lambda: self.button_event(btn1)
        )
        self.btn_1.grid(row=1, column=0, padx=(40, 5), pady=10, sticky="w")

        self.btn_2 = ctk.CTkButton(
            self, text=btn2, **LINK_BTN, command=lambda: self.button_event(btn2)
        )
...
class CTkNotification(ctk.CTkFrame):
    def __init__(
        ...
        self.message_label = ctk.CTkLabel(
            self, text=f"  {message}", font=("", 13), image=self.icon, compound="left"
        )
...
class CTkCard(ctk.CTkFrame):
    def __init__(self, master: any, border_width=1, corner_radius=5, **kwargs):
        super().__init__(
            master, border_width=border_width, corner_radius=corner_radius, **kwargs
        )
        self.grid_propagate(False)
    ...
    def card_1(
        ...
        card_text = ctk.CTkLabel(
            self, text=text, font=("", 13), wraplength=wrap_length, justify="left"
        )
        card_text.grid(row=2, column=0, padx=10, pady=5, sticky="nw")

        card_button = ctk.CTkButton(
            self, text=button_text, height=35, command=command if command else None
        )
    ...
    def card_2(
        ...
        subtitle="Subtitle",
        text=TEXT,
        link1_text="Card link1",
        link2_text="Card link2",
        command1=None,
        command2=None,
    ):
        self.configure(width=width, height=height)
        ...
        card_link1 = ctk.CTkButton(
            self, text=link1_text, **BTN_LINK, command=command1 if command1 else None
        )
        card_link1.grid(row=3, column=0, padx=5, pady=10, sticky="w")
        card_link2 = ctk.CTkButton(
            self, text=link2_text, **BTN_LINK, command=command2 if command2 else None
        )
    ...
    def card_3(
        ...
        ctk.CTkFrame(self, height=2, fg_color=("#C9CCD6", "#5A5D63")).grid(
            row=1, column=0, padx=0, pady=2, sticky="ew"
        )
...
class CTkCarousel(ctk.CTkFrame):
    def __init__(
        ...
        **kwargs,
        ...
        self.prev_icon = ctk.CTkImage(
            Image.open(ICON_PATH["left"]), Image.open(ICON_PATH["left"]), (30, 30)
        )
        self.next_icon = ctk.CTkImage(
            Image.open(ICON_PATH["right"]), Image.open(ICON_PATH["right"]), (30, 30)
        )
    ...
    def next_callback(self):
        ...
        next_image = ctk.CTkImage(
            create_rounded, create_rounded, (self.width, self.height)
        )
...
class CTkInput(ctk.CTkEntry):
    def __init__(self, master: any, icon_width=20, icon_height=20, **kwargs):
        ...
        self.warning = ctk.CTkImage(
            Image.open(ICON_PATH["warning2"]),
            Image.open(ICON_PATH["warning2"]),
            (self.icon_width, self.icon_height),
        )
        self.eye1 = ctk.CTkImage(
            Image.open(ICON_PATH["eye1"][0]),
            Image.open(ICON_PATH["eye1"][1]),
            (self.icon_width, self.icon_height),
        )
        self.eye2 = ctk.CTkImage(
            Image.open(ICON_PATH["eye2"][0]),
            Image.open(ICON_PATH["eye2"][1]),
            (self.icon_width, self.icon_height),
        )
    ...
    def reset_default(self):
        ...
        for widget in self.winfo_children():
            ...
            if widget_name.startswith("!ctklabel") or widget_name.startswith(
                "!ctkbutton"
            ):
                widget.destroy()
...
class CTkLoader(ctk.CTkFrame):
    def __init__(
        self, master: any, opacity: float = 0.8, width: int = 40, height: int = 40
    ):
        self.master = master
        self.master.update()
        ...
        super().__init__(
            master, width=self.master_width, height=self.master_height, corner_radius=0
        )
...
class CTkTreeview(ctk.CTkFrame):
    def __init__(self, master: any, items):
        ...
        self.bg_color = self.root._apply_appearance_mode(
            ctk.ThemeManager.theme["CTkFrame"]["fg_color"]
        )
        self.text_color = self.root._apply_appearance_mode(
            ctk.ThemeManager.theme["CTkLabel"]["text_color"]
        )
        self.selected_color = self.root._apply_appearance_mode(
        ...
        self.img_close = ImageTk.PhotoImage(
            self.im_close, name="img_close", size=(15, 15)
        )
        self.img_empty = ImageTk.PhotoImage(
            self.im_empty, name="img_empty", size=(15, 15)
        )
